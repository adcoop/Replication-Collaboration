...
## the stuff that is commented out is just testing code!
# 
...
ri <- function(dat, Z, Y, cov = NULL, prob = NULL, iter = 10000, ipw = TRUE) {
  ...
  block <- read.dta13('~/Dropbox/Green and Vasudevan (2015) replication/0. Randomization/Output Data/stations_sample_randomized.dta')
  ...
  if (ipw == T){
    ...
    ate <- summary(lm(Y ~ Z + cov))$coef['Z',1]
  }
  results$ate <- ate
